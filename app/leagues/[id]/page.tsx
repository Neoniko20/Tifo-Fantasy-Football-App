...
export default function LeagueSpieltagPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rankColor = (i: number) =>
    i === 0 ? "#f5a623" : i === 1 ? "#c8b080" : i === 2 ? "#a07040" : "#3a2a10";
  ...
  return (
    <main className="flex min-h-screen flex-col items-center pb-28"
      ...
      <div className="fixed top-0 left-1/2 -translate-x-1/2 w-64 h-32 rounded-full blur-3xl opacity-8 pointer-events-none"
        style={{ background: "#f5a623", zIndex: 49 }} />
      ...
      <div className="w-full max-w-md px-4">
        ...
        {league?.status === "setup" && (
          <div className="mt-4 space-y-3">
            <div className="rounded-2xl p-5 text-center"
              style={{ background: "#141008", border: "1px solid #f5a62330" }}>
              <p className="text-3xl mb-3">📋</p>
              <p className="text-base font-black mb-1" style={{ color: "#f5a623" }}>Draft vorbereiten</p>
            ...
            {(draftSession || teams.length > 0) && (
              <div className="rounded-2xl p-4"
                ...
                <div className="grid grid-cols-2 gap-y-3">
                  <div>
                    <p className="text-[7px] font-black uppercase tracking-widest" style={{ color: "#3a2a10" }}>Modus</p>
                  ...
                  <div>
                    <p className="text-[7px] font-black uppercase tracking-widest" style={{ color: "#3a2a10" }}>Teams</p>
                    <p className="text-xs font-black" style={{ color: "#c8b080" }}>{teams.length}</p>
                  </div>
                  {draftSession?.rounds && (
                    <div>
                      <p className="text-[7px] font-black uppercase tracking-widest" style={{ color: "#3a2a10" }}>Runden</p>
                      <p className="text-xs font-black" style={{ color: "#c8b080" }}>{draftSession.rounds}</p>
                    </div>
                  )}
                  {draftSession?.picks_per_round && (
                    <div>
                      <p className="text-[7px] font-black uppercase tracking-widest" style={{ color: "#3a2a10" }}>Picks/Runde</p>
                      <p className="text-xs font-black" style={{ color: "#c8b080" }}>{draftSession.picks_per_round}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  {teams.map((t: any, i: number) => (
                    <div key={t.id} className="flex items-center gap-2">
                      <span className="text-[8px] font-black w-4 text-right" style={{ color: "#3a2a10" }}>{i + 1}</span>
                      <p className="text-xs font-black flex-1"
                        style={{ color: t.user_id === user?.id ? "#f5a623" : "#c8b080" }}>
                        {t.name}
                        {t.user_id === user?.id && <span className="ml-1 text-[7px]" style={{ color: "#f5a623" }}>(Du)</span>}
        ...
        {league?.status === "drafting" && (
          <div className="mt-4 space-y-3">
            <div className="rounded-2xl p-5"
              style={{ background: "#141008", border: "1px solid #00ce7d40" }}>
              <div className="flex items-center gap-3 mb-3">
                <span className="w-2.5 h-2.5 rounded-full flex-shrink-0 animate-pulse"
                  style={{ background: "#00ce7d" }} />
                <p className="text-sm font-black" style={{ color: "#00ce7d" }}>Draft läuft!</p>
              </div>
              <p className="text-[9px] font-black uppercase tracking-widest mb-1" style={{ color: "#5a4020" }}>
            ...
            <a href={`/leagues/${leagueId}/draft`}
              ...
              style={{ background: "#00ce7d", color: "#0c0900" }}>
              Zum Draft →
            </a>
          </div>
        )}
        ...
        {league?.status !== "setup" && league?.status !== "drafting" && gameweeks.length === 0 && (
          <div className="rounded-2xl p-8 text-center mt-6"
            ...
            <p className="text-sm font-black mb-1" style={{ color: "#c8b080" }}>Noch keine Spieltage</p>
            <p className="text-[9px] mb-4" style={{ color: "#5a4020" }}>Der Liga-Owner kann Spieltage im Admin anlegen</p>
                ...
                <button key={gw.gameweek} onClick={() => setSelectedGW(gw.gameweek)}
                  ...
                  {gw.status === "active" && (
                    <span className="ml-1 w-1.5 h-1.5 rounded-full inline-block"
                      style={{ background: "#00ce7d", verticalAlign: "middle" }} />
                  )}
                </button>
              ))}
            </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1 text-center">
                    <p className="font-black text-sm truncate"
                      style={{ color: myMatchup.home?.id === myTeam?.id ? "#f5a623" : "#c8b080" }}>
                      {myMatchup.home?.name}
                    </p>
                    <p className="text-[7px] font-black uppercase mt-0.5" style={{ color: "#5a4020" }}>
                      {myMatchup.home?.user_id ? "" : "Bot"}
                    </p>
                  </div>
                  <div className="px-4 text-center">
                    <p className="text-xl font-black" style={{ color: "#f5a623" }}>
                      {myMatchup.home_points?.toFixed(1) ?? "—"}
                      <span className="mx-2 text-sm" style={{ color: "#3a2a10" }}>:</span>
                      {myMatchup.away_points?.toFixed(1) ?? "—"}
                    </p>
                    {myMatchup.winner_id && (
                      <p className="text-[7px] font-black uppercase mt-0.5" style={{ color: "#00ce7d" }}>
                  ...
                  <div className="flex-1 text-center">
                    <p className="font-black text-sm truncate"
                      style={{ color: myMatchup.away?.id === myTeam?.id ? "#f5a623" : "#c8b080" }}>
                      {myMatchup.away?.name}
                    </p>
                    <p className="text-[7px] font-black uppercase mt-0.5" style={{ color: "#5a4020" }}>
            ...
            <div className="flex gap-1.5 mb-4">
              {([
                ...
                <button key={id} onClick={() => setView(id)}
                  ...
                  style={{
                    background: view === id ? "#f5a623" : "#141008",
                    color: view === id ? "#0c0900" : "#5a4020",
                    border: `1px solid ${view === id ? "#f5a623" : "#2a2010"}`,
                  }}>
                  {label}
                </button>
              ))}
                ...
                {teams.map((team, i) => {
                  ...
                  return (
                    <div key={team.id}
                      ...
                      style={{
                        background: isMine ? "#1a1208" : "#141008",
                        border: `1px solid ${isMine ? "#3a2a10" : "#2a2010"}`,
                      }}
                      onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = "#f5a623"}
                      onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = isMine ? "#3a2a10" : "#2a2010"}>
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="font-black text-base" style={{ color: i === 0 ? "#f5a623" : "#c8b080" }}>
                          {(team.total_points ?? 0).toFixed(1)}
                        </p>
                        <p className="text-[7px] font-black uppercase" style={{ color: "#3a2a10" }}>Pts</p>
                      </div>
                    </div>
                  );
                })}
                ...
                {isH2H ? (
                  matchups.length === 0 ? (
                    <div className="rounded-xl p-6 text-center"
                      ...
                      {league?.owner_id === user?.id && (
                        <p className="text-[8px] mt-2" style={{ color: "#3a2a10" }}>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0 text-center">
                              <p className="font-black text-xs truncate"
                                style={{ color: homeIsMine ? "#f5a623" : "#c8b080" }}>
                                {m.home?.name}
                              </p>
                            </div>
                            <div className="px-3 text-center flex-shrink-0">
                              <p className="text-sm font-black" style={{ color: "#f5a623" }}>
                                ...
                                <span className="mx-1.5 text-xs" style={{ color: "#3a2a10" }}>:</span>
                                {m.away_points !== null ? m.away_points?.toFixed(1) : "—"}
                              </p>
                              {m.winner_id && (
                                <p className="text-[7px] font-black uppercase" style={{ color: "#00ce7d" }}>Final</p>
                              )}
                            </div>
                            <div className="flex-1 min-w-0 text-center">
                              <p className="font-black text-xs truncate"
                                style={{ color: awayIsMine ? "#f5a623" : "#c8b080" }}>
                                {m.away?.name}
                              </p>
                            </div>
                          </div>
                      ...
                      <div key={team.id}
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="font-black text-base"
                            style={{ color: team.gw_points !== null ? "#c8b080" : "#2a2010" }}>
                            {team.gw_points !== null ? team.gw_points.toFixed(1) : "—"}
                          </p>
                          <p className="text-[7px] font-black uppercase" style={{ color: "#3a2a10" }}>GW-Pts</p>
                        </div>
                      </div>
                    );
                  })
