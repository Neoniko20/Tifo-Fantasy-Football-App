...
export function GameweekView({ leagueId }: GameweekViewProps) {
  ...
  useEffect(() => {
    const activeGW = gameweeks.find((g: any) => g.gameweek === selectedGW);
  ...
  async function loadGWDataWithTeams(gw: number, allTeams: any[]) {
    ...
    const ranked = allTeams
      .map((t: any) => ({ ...t, gw_points: teamGWPts[t.id] || 0 }))
      .sort((a: any, b: any) => b.gw_points - a.gw_points);
    setGwPoints(ranked);
  ...
  const activeGWData = gameweeks.find((g: any) => g.gameweek === selectedGW);
    ...
    <main className="flex min-h-screen flex-col items-center p-4 pb-28" style={{ background: "var(--bg-page)" }}>
      ...
      {gameweeks.length === 0 ? (
        ...
        <>
            ...
            {gameweeks.map((gw: any) => {
              ...
              return (
                <button key={gw.gameweek} onClick={() => setSelectedGW(gw.gameweek)}
                  ...
                  GW{gw.gameweek}
                  {gw.status === "active" && (
                    <span className="ml-1" style={{ color: selectedGW === gw.gameweek ? "var(--bg-page)" : "var(--color-primary)" }}>●</span>
                  )}
                  {hasDouble && selectedGW !== gw.gameweek && (
                    <span className="ml-0.5 text-[7px]" style={{ color: "var(--color-primary)" }}>×2</span>
                  )}
                </button>
              );
            })}
          ...
          {/* GW-Info + Liga-Status */}
          {(() => {
            const gw = gameweeks.find((g: any) => g.gameweek === selectedGW);
            if (!gw) return null;
          ...
          {tab === "ranking" && (
            <div className="w-full max-w-md space-y-2">
              ...
              ) : gwPoints.map((team: any, i: number) => (
            ...
            <div className="w-full max-w-md space-y-3">
              ...
              {matchups.length === 0 ? (
                <div className="rounded-2xl p-6 text-center"
                  ...
                  <p className="font-black text-sm mb-1" style={{ color: "var(--color-text)" }}>Noch keine Paarungen</p>
