...
export default function AccountPage() {
  ...
  if (section === "username") return (
    <SubSection title="Benutzername" onBack={() => setSection("main")}>
      ...
      <input
        ...
        style={{ background: "#1a1208", border: "1px solid #3a2a10", color: "#c8b080" }}
        maxLength={24}
        autoFocus
      />
  ...
  if (section === "email") return (
    <SubSection title="E-Mail ändern" onBack={() => setSection("main")}>
      ...
      <p className="text-sm font-black mb-4" style={{ color: "#5a4020" }}>{user?.email}</p>
      <input
        value={newEmail}
        ...
        autoFocus
      />
      <p className="text-[8px] mb-4" style={{ color: "#3a2a10" }}>
    ...
    <SubSection title="Passwort ändern" onBack={() => setSection("main")}>
      {[
        ...
        <div key={label} className="mb-3">
          <p className="text-[8px] font-black uppercase tracking-widest mb-1" style={{ color: "#3a2a10" }}>{label}</p>
          <input
            value={val}
    ...
    <SubSection title="Benachrichtigungen" onBack={() => setSection("main")}>
      ...
      {notifPermission !== "granted" && (
        <div className="rounded-2xl p-4 mb-4"
          ...
          <p className="text-[9px] font-black mb-2" style={{ color: "#c8b080" }}>
      ...
      {notifPermission === "granted" && (
        <div className="flex items-center gap-2 mb-4 px-3 py-2 rounded-xl"
          ...
          <span className="w-1.5 h-1.5 rounded-full" style={{ background: "#00ce7d" }} />
          <p className="text-[8px] font-black uppercase tracking-widest" style={{ color: "#00ce7d" }}>
            Push aktiviert
          </p>
        </div>
      )}
        ...
        {NOTIF_LABELS.map(({ key, label, desc }) => (
          <div key={key}
            ...
            <div className="flex-1 min-w-0 pr-4">
              <p className="text-xs font-black" style={{ color: "#c8b080" }}>{label}</p>
              <p className="text-[8px] mt-0.5" style={{ color: "#5a4020" }}>{desc}</p>
            </div>
            <button
              onClick={() => toggleNotif(key)}
              className="flex-shrink-0 relative w-10 h-5 rounded-full transition-all"
              style={{
                background: notifs[key] ? "#f5a623" : "#2a2010",
              }}>
              <span
                className="absolute top-0.5 w-4 h-4 rounded-full transition-all"
                style={{
                  background: notifs[key] ? "#0c0900" : "#5a4020",
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="relative mb-3">
            <div className="w-20 h-20 rounded-full overflow-hidden flex items-center justify-center"
              style={{ border: "3px solid #f5a623", background: "#141008" }}>
              {avatarUrl ? (
                <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
              ) : (
                <span className="text-3xl font-black" style={{ color: "#f5a623" }}>{initial}</span>
              )}
              {uploadingAvatar && (
                <div className="absolute inset-0 flex items-center justify-center rounded-full"
                  style={{ background: "rgba(12,9,0,0.7)" }}>
                  <span className="text-[8px] font-black animate-pulse" style={{ color: "#f5a623" }}>...</span>
                </div>
              )}
            </div>
            <button
              ...
              style={{ background: "#f5a623", border: "2px solid #0c0900" }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#0c0900" strokeWidth="2.5">
          ...
          <p className="font-black text-base" style={{ color: "#c8b080" }}>{displayName}</p>
          <p className="text-[9px] mt-0.5" style={{ color: "#5a4020" }}>{user?.email}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <SectionGroup label="Benachrichtigungen">
            <SettingsRow
              ...
              valueColor={notifPermission === "granted" ? "#00ce7d" : notifPermission === "denied" ? "#ff4d6d" : "#5a4020"}
          ...
          <SectionGroup label="App">
            <div className="px-4 py-3 flex items-center justify-between">
              <p className="text-xs font-black" style={{ color: "#5a4020" }}>Version</p>
              <p className="text-xs font-black" style={{ color: "#3a2a10" }}>1.0.0</p>
            </div>
            <div style={{ borderTop: "1px solid #1a1208" }}>
              <button
                ...
                <p className="text-xs font-black" style={{ color: "#5a4020" }}>Support kontaktieren</p>
          ...
          <button
            ...
            style={{ background: "#1a0808", color: "#ff4d6d", border: "1px solid #ff4d6d30" }}>
            Abmelden
          </button>
        </div>
      </div>
...
function SaveButton({
  ...
  return (
    <button onClick={onClick} disabled={saving}
      ...
      style={{ background: saving ? "#2a1a00" : "#f5a623", color: "#0c0900" }}>
